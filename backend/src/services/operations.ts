...
export function needsOperationalAlert(queue: QueueHealth): boolean {
  return queue.deadLetter > 0 || queue.oldestAgeSeconds > 300;
}

async function durableQueueHealth(table: "webhook_events" | "room_snapshot_outbox"): Promise<QueueHealth> {
  const rows = await sql<{
    ...
    SELECT
      ...
      COALESCE(extract(epoch FROM now() - min(created_at) FILTER (WHERE status IN ('pending', 'failed'))), 0)::text AS oldest_age_seconds
    FROM ${sql(table)}
  `;
  const row = rows[0];
  return {
...
export async function operationsSnapshot() {
  const [billingWebhooks, roomSnapshots, leaderboardRows, openRooms] = await Promise.all([
